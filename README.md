...
- ✅ **F-1 터널링** — 릴레이↔커넥터 아웃바운드 WSS 터널, 자동 재연결 (실환경 검증 완료)
- ✅ **F-2 인증 (임시)** — 데모 계정 선택형 로그인 + 세션 쿠키 (추후 Google OAuth로 교체)
- ✅ **F-3 정적 정책** — `relay/policy.json` 사용자↔앱 매핑, 미허용 앱 403 차단. 파일 수정 시 자동 반영
- ✅ **F-7 목업 사내 시스템** — 그룹웨어/위키/인사DB
- ⬜ F-4 AI 위험도 평가 · F-5 감사 로그 · F-6 관리자 대시보드

### 데모 계정 (relay/policy.json)
...
사내망 정책상 **443만 인바운드 허용** — 릴레이는 443으로 띄운다.

```bash
cd /data/hackertone-amd && git pull
pkill -f "relay/server.js"; pkill -f "connector/index.js"; pkill -f "mock-apps/server.js"

nohup node mock-apps/server.js > /tmp/mock.log 2>&1 &
RELAY_PORT=443 nohup node relay/server.js > /tmp/relay.log 2>&1 &
RELAY_URL=ws://localhost:443/tunnel nohup node connector/index.js > /tmp/connector.log 2>&1 &
```

브라우저: `http://groupware.10.52.249.249.sslip.io:443` (wiki / hrdb 동일 패턴)
